...
base_grid_c<-basething+geom_polygon(data=grid$all_lines_c[nrow(grid$all_lines_c):1,],aes(x,y,group=rev(q)),
                                    fill=c(rep(rep(rev(bg_colors),2),each=n),rep('white',n)))+
                       geom_polygon(data=grid$lines_circle$`0% Quantile`,aes(x,y),fill="white")+
                       geom_path(data=grid$axis_path,aes(x=x,y=y,group=axis.no),
                                 colour=axis.line.colour,alpha=0.4)

### 3.5 Add text to gg-object
font_size=2
base_text<-base_grid_c+geom_text(data=grid$all_lines,aes(x,y,label=values),size=font_size)+
...
gg<-base_text+geom_path(data=xy_lines,aes(x=x,y=y,group=group,color=group),alpha=0.8,size=1)+
              theme(legend.position="bottom") # leave this line out if the legend should be right
